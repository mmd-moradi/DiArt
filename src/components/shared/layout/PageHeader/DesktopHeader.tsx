...
const DesktopHeader = ({ session, title }: Props) => {
  const { state: showForm, dispatch: setShowForm } =
  ...
  return (
    <>
      {showForm && <AuthForm show={showForm} setShowForm={setShowForm} />}
      <header
        className={`px-8 py-4 sticky top-0 w-full bg-primary z-[90] md:px-16 ${headerStyle}`}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3.5">
            <button
              onClick={() => router.push("/")}
              className="flex items-center text-white p-3.5 rounded-full bg-gradient-primary-1 cursor-pointer font-bold transition-all duration-200 ease-in-out hover:bg-gradient-primary-3"
            >
              <FaArrowLeft />
            </button>
          ...
          <div className="flex items-center space-x-4">
            {session ? (
              <button
                className="flex items-center gap-2 rounded-lg bg-gradient-primary-1 px-3 py-2 outline-none border-none cursor-pointer transition-all duration-300 ease-in-out hover:bg-gradient-primary-3"
                type="button"
                onClick={() => signOut()}
              >

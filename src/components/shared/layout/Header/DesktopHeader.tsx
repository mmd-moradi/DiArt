...
const DesktopHeader = ({ session }: Props) => {
  ...
  const handleAuthenticatedNavigation = (href: string) => {
    if (session) {
      ...
      setShowForm({ type: "AuthFormOpened" });
      ...
      <header className="px-8 py-2 sticky top-0 left-0 w-full bg-primary z-[95] md:px-16">
        <div className="flex items-center justify-between">
          ...
          <nav className="flex items-center justify-center gap-8">
            <Link href="/arts" style={{ textDecoration: "none" }} passHref>
            ...
            <button onClick={() => handleAuthenticatedNavigation("/create")}>
              <span className="text-white text-lg font-semibold w-full transition-all duration-200 ease-in-out hover:text-secondary">
                Create
              </span>
            </button>
            <Link href="/activity" style={{ textDecoration: "none" }} passHref>
              <span className="text-white text-lg font-semibold w-full transition-all duration-200 ease-in-out hover:text-secondary">
                Activity
              </span>
            </Link>
